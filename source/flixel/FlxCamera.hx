...
class FlxCamera extends FlxBasic
{
	...
	@:noCompletion
	public function startQuadBatch(graphic:FlxGraphic, colored:Bool, hasColorOffsets:Bool = false, ?blend:BlendMode, smooth:Bool = false, ?shader:FlxShader, ?csData:CSData)
	{
		...
		if (_currentDrawItem != null
			...
			&& (_headTiles.csData != null && _headTiles.csData.equals(csData))
		)
		{
			return _headTiles;
		}
	...
	public function drawPixels(?frame:FlxFrame, ?pixels:BitmapData, matrix:FlxMatrix, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false,
			?shader:FlxShader, ?csData:CSData):Void
	{
		if (FlxG.renderBlit)
		{
		...
		else
		{
			...
			#else
			var drawItem = startQuadBatch(frame.parent, isColored, hasColorOffsets, blend, smoothing, shader, csData);
			#end
