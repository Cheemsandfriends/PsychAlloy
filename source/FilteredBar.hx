...
/**
 * FilteredBar is a quick and easy way to create a graphical bar which can
 ...
 */
class FilteredBar extends FlxFilteredSprite
{
	/**
	...
	/**
	 * The positionOffset controls how far offset the FilteredBar is from the parent sprite (if at all)
	 */
	public var positionOffset(default, null):FlxPoint;

	/**
	 * If this FilteredBar should be killed when its empty
	 */
	...
	/**
	 * Number of frames FilteredBar will have. Default value is 100.
	...
	/**
	 * Create a new FilteredBar Object
	 *
	...
	/**
	 * Sets a parent for this FilteredBar. Instantly replaces any previously set parent and refreshes the bar.
	 *
	 * @param	parentRef	A reference to an object in your game that you wish the bar to track
	 * @param	variable	The variable of the object that is used to determine the bar position. For example if the parent was an FlxSprite this could be "health" to track the health value
	 * @param	track		If you wish the FilteredBar to track the x/y coordinates of parent set to true (default false)
	 * @param	offsetX		The offset on X in relation to the origin x/y of the parent
	 * @param	offsetY		The offset on Y in relation to the origin x/y of the parent
	 */
	...
	/**
	 * Sets callbacks which will be triggered when the value of this FilteredBar reaches min or max.
	 * Functions will only be called once and not again until the value changes.
	 * Optionally the FilteredBar can be killed if it reaches min, but if will fire the empty callback first (if set)
	 *
	 * @param	onEmpty			The function that is called if the value of this FilteredBar reaches min
	 * @param	onFilled		The function that is called if the value of this FilteredBar reaches max
	 * @param	killOnEmpty		If set it will call FilteredBar.kill() if the value reaches min
	 */
	...
	/**
	 * Set the minimum and maximum allowed values for the FilteredBar
	 *
	 * @param	min			The minimum value. I.e. for a progress bar this would be zero (nothing loaded yet)
	 * @param	max			The maximum value the bar can reach. I.e. for a progress bar this would typically be 100.
	 */
	public function setRange(min:Float, max:Float):Void
	{
		if (max <= min)
		{
			throw "FilteredBar: max cannot be less than or equal to min";
			return;
		}
	 ...
	 * @return	This FilteredBar object with generated images for front and background.
	 */
	public function createFilledBar(empty:FlxColor, fill:FlxColor, showBorder:Bool = false, border:FlxColor = FlxColor.WHITE):FilteredBar
	{
	 ...
	 * @param	showBorder		Should the bar be outlined with a 1px solid border?
	 * @param	border			The border colour in 0xAARRGGBB format
	 * @return	This FilteredBar object with generated image for rendering health bar background.
	 */
	public function createColoredEmptyBar(empty:FlxColor, showBorder:Bool = false, border:FlxColor = FlxColor.WHITE):FilteredBar
	{
		if (FlxG.renderTile)
		{
	 ...
	 * @param	showBorder	Should the bar be outlined with a 1px solid border?
	 * @param	border		The border colour in 0xAARRGGBB format
	 * @return	This FilteredBar object with generated image for rendering actual values.
	 */
	public function createColoredFilledBar(fill:FlxColor, showBorder:Bool = false, border:FlxColor = FlxColor.WHITE):FilteredBar
	{
		if (FlxG.renderTile)
		{
	...
	public function createGradientBar(empty:Array<FlxColor>, fill:Array<FlxColor>, chunkSize:Int = 1, rotation:Int = 180, showBorder:Bool = false,
			border:FlxColor = FlxColor.WHITE):FilteredBar
	{
	 ...
	 * @param	showBorder		Should the bar be outlined with a 1px solid border?
	 * @param	border			The border colour in 0xAARRGGBB format
	 * @return 	This FilteredBar object with generated image for background rendering.
	 */
	...
	public function createImageBar(?empty:FlxGraphicAsset, ?fill:FlxGraphicAsset, emptyBackground:FlxColor = FlxColor.BLACK,
			fillBackground:FlxColor = FlxColor.LIME):FilteredBar
	{
	 ...
	 * @return	This FilteredBar object with generated image for background rendering.
	 */
	public function createImageEmptyBar(?empty:FlxGraphicAsset, emptyBackground:FlxColor = FlxColor.BLACK):FilteredBar
	{
		if (empty != null)
		{
	 ...
	 * @return	This FilteredBar object with generated image for rendering actual values.
	 */
	public function createImageFilledBar(?fill:FlxGraphicAsset, fillBackground:FlxColor = FlxColor.LIME):FilteredBar
	{
		if (fill != null)
		{
